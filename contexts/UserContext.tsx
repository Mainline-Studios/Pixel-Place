...
import { setSessionCookie, getSessionCookie, clearSessionCookie } from '@/lib/cookies';
...
export function UserProvider({ children }: { children: React.ReactNode }) {
  // Restore user from cookie or sessionStorage on mount
  const getInitialUser = async (): Promise<User | null> => {
    if (typeof window === 'undefined') return null;
    try {
      // Check cookie first (persists across tabs/restarts), then sessionStorage
      const savedUsername = getSessionCookie() || sessionStorage.getItem('pixelPlaceLoggedInUser');
      if (savedUsername) {
  ...
  // Persist user to cookie + sessionStorage whenever it changes
  useEffect(() => {
