...
import { initializeStorage, getUsers, saveUsers, isUserBanned, getBanForUser, checkDeviceBanStatus } from '@/lib/storage';
...
export function UserProvider({ children }: { children: React.ReactNode }) {
  ...
  const createAccount = async (username: string, password: string, gender: string): Promise<{ success: boolean; message: string }> => {
    ...
    try {
      ...
      return { success: false, message: regData.error || 'Registration failed. Please try again.' };
    } catch (_e) {
      return { success: false, message: 'Could not reach server. Try again when online.' };
    }
  };
