...
export default function HolidayBundle({ user, onClose }: HolidayBundleProps) {
    ...
    const rewards = holiday ? generateHolidayRewards(holiday) : [];
        ...
        if (savedState) {
            try {
                ...
                if (state.selectedReward) {
                    setSelectedReward(state.selectedReward);
                }
            } catch (e) {
    ...
    // Save purchase state
    const savePurchaseState = (purchased: boolean, spun: boolean, reward: HolidayReward | null = null) => {
        if (!holiday) return;
        const state = {
        ...
        localStorage.setItem(`holiday_bundle_${user.username}_${holiday.name}`, JSON.stringify(state));
    };
