...
//
// Notes:
// - This is an in-memory server for demo/prototyping. It enforces one vote per WebSocket connection per vote round.
// - For production, add authentication, persistence, TLS (wss), rate limiting, and authorization for starting votes.
...
// track which connection (by ws) has voted this round to prevent double-votes per connection
const voters = new Map(); // ws -> mapKey
...
wss.on('connection', (ws) => {
  ...
  ws.on('message', (raw) => {
    ...
    switch (data.type) {
      ...
      case 'start_vote': {
        // Simple policy: anyone can start a vote. In production, require privileged clients or auth.
      ...
      case 'cast_vote': {
        if (!votingActive) {
          ws.send(JSON.stringify({ type: 'error', message: 'no active vote' }));
          break;
        }
        const map = data.map;
        if (!map || !Object.prototype.hasOwnProperty.call(votes, map)) {
          ws.send(JSON.stringify({ type: 'error', message: 'invalid map' }));
          break;
        }
        if (voters.has(ws)) {
  ...
  ws.on('close', () => {
    // We keep votes even if clients disconnect. If desired, you could remove that client's vote on disconnect.
  });
});
