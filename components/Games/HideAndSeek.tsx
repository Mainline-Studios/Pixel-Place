  ...
  ws.on('message', (msg) => {
    ...
    if (t === 'join'){
      ...
      if (Object.keys(room.players).length >= room.minPlayers && room.round === 0) startRound(room);
        ...
        const simpleList = Object.values(room.players).map(p => ({ id:p.id, name:p.name }));
        broadcast(room, { type:'player-list', players: simpleList, minPlayers: room.minPlayers, maxPlayers: room.maxPlayers });
      }
      return;
    }
    ...
    if (t === 'action'){
      ...
      if (action === 'move'){
        ...
        room.clients.forEach((targetPid, clientWs) => {
          const plist = playerListFor(targetPid, room);
          sendToWs(clientWs, { type:'state', players: plist });
        });
