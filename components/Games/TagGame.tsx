...
export default function TagGame({ onClose }: TagGameProps) {
  ...
  const createPlayer = (id: string, name: string, isCPU: boolean, isIt: boolean = false): Player => {
    return {
      id,
      name,
  ...
  const initializeGame = (count: number, startWithCPU: boolean) => {
    ...
    // Add human player
    const humanPlayer = createPlayer('player1', 'You', false, count === 1);
    newPlayers.push(humanPlayer);

    // Add CPU players
    if (startWithCPU || count > 1) {
      for (let i = 2; i <= count; i++) {
        const cpuPlayer = createPlayer(`cpu${i}`, `CPU ${i - 1}`, true, count === 1 && i === 2);
  ...
  const updateCPUPlayer = useCallback((player: Player, allPlayers: Player[]): Player => {
  ...
  const checkTag = useCallback((allPlayers: Player[]): Player[] => {
    const itPlayer = allPlayers.find(p => p.isIt);
    ...
    return allPlayers.map(player => {
      ...
      if (player.id === itPlayer.id && allPlayers.some(p => !p.isIt && Math.sqrt((p.x - itPlayer.x) ** 2 + (p.y - itPlayer.y) ** 2) < TAG_DISTANCE)) {
        return {
          ...player,
          isIt: false,
        };
  ...
  }, [gameState, updateCPUPlayer, checkTag]);
    ...
    <div style={{
      ...
      <div style={{ display: 'flex', justifyContent: 'space-between', alignItems: 'center', marginBottom: '16px' }}>
        ...
        <div style={{ display: 'flex', gap: '8px' }}>
          <button className="btn" onClick={() => setGameState(gameState === 'paused' ? 'playing' : 'paused')} style={{ padding: '6px 12px', fontSize: '12px' }}>
