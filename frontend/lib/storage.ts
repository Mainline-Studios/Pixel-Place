...
/** Normalize usernames so "Administrator 1237" matches the built-in ops account. */
export function adminUsernameKey(username: string): string {
  return (username || '').replace(/\s+/g, '').toLowerCase();
}

export function isListedHeadAdmin(username: string): boolean {
  const k = adminUsernameKey(username);
  return HEAD_ADMIN_USERNAMES.some((h) => adminUsernameKey(h) === k);
}

export function isListedAdminAccount(username: string): boolean {
  const k = adminUsernameKey(username);
  return ADMIN_ACCOUNTS.some((a) => adminUsernameKey(a.username) === k);
}

/** Primary ops account (matches `Administrator1237` with or without spaces). */
export function isPrimaryOpsAdmin(username: string): boolean {
  return adminUsernameKey(username) === adminUsernameKey('Administrator1237');
}

export function findAdminAccountByUsername(username: string): (typeof ADMIN_ACCOUNTS)[number] | undefined {
  const k = adminUsernameKey(username);
  return ADMIN_ACCOUNTS.find((a) => adminUsernameKey(a.username) === k);
}
...
export async function getAuditLogs(): Promise<AuditLogEntry[]> {
  if (typeof window === 'undefined') return [];
  try {
    const response = await authenticatedFetch(apiUrl('/api/moderation/audit?limit=200'));
...
export async function postModerationAction(payload: ModerationActionPayload): Promise<boolean> {
  if (typeof window === 'undefined') return false;
  try {
    const response = await authenticatedFetch(apiUrl('/api/moderation/action'), {
      method: 'POST',
      headers: { 'Content-Type': 'application/json' },
