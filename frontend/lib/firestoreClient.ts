...
function userFromDoc(d: { id: string } & Record<string, unknown>): User {
  ...
  return {
    ...
    role: ((doc.role as string) || 'user') as User['role'],
...
function gameFromDoc(d: { id: string } & Record<string, unknown>): { id: string; title: string; desc: string; owner: string; ts: number; sceneData: { objects: unknown[] }; publishedBy?: string; gameType?: string; fileContent?: string; fileType?: string; gameCode?: string; thumbnail?: string } {
  const doc = d as Record<string, unknown>;
...
export function subscribeToUserMadeGames(
  callback: (games: Array<{ id: string; title: string; desc: string; owner: string; ts: number; sceneData: unknown; thumbnail?: string; gameCode?: string; gameType?: string }>) => void
): () => void {
  const database = getDb();
  if (!database) {
    callback([]);
