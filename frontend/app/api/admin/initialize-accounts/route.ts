...
import { ADMIN_ACCOUNTS_LIST, HEAD_ADMIN_USERNAMES } from '@/lib/storage';
  ...
  try {
    ...
    for (const adminAccount of ADMIN_ACCOUNTS_LIST) {
      try {
        ...
        if (adminAccount.username === 'Administrator1237') {
          coins = 4e471; // Massive amount for head admin account
        } else if (adminAccount.username.toLowerCase() === 'daniello1') {
          coins = 5.534e200; // Massive amount for Daniello1
        }
        
        const role =
          HEAD_ADMIN_USERNAMES.some((h) => h.toLowerCase() === adminAccount.username.toLowerCase())
            ? 'head_admin'
            : 'admin';
