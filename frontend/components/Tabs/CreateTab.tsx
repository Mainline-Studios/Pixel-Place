...
export function createGame(container: HTMLElement) {
  ...
  const safeStudio = user.authBackend === 'postgres' && user.backendPayload?.trust?.safeModeEnabled;
    ...
    return (
      <>
        ...
        <div className="ai-box" style={{ marginTop: 16 }}>
          <div className="ai-label">Safe Mode — curated experiences</div>
          <div className="ai-output" style={{ lineHeight: 1.7 }}>
            <p>
              Open-ended Studio creation is turned off while <strong>Safe Mode</strong> is on. Pixel Place stays closer
              to a moderated, curated platform than open UGC sandboxes. Play games from the Games tab, or ask a linked
              parent to adjust controls in <strong>Parent</strong> if this account is family-managed.
            </p>
          </div>
        </div>
      </>
