...
import { User, FriendRequest, Message, Skin } from '@/types';
import { getUsers, getSkins } from '@/lib/storage';
...
function normalizeSkinForPreview(skin: Skin): Skin {
  return {
    ...skin,
    colors: {
...
function FriendSkinAvatar({ profileUser, catalog }: { profileUser: User; catalog: Skin[] }) {
  const id = profileUser.equippedSkin || 'starter_classic';
  const raw =
    catalog.find((s) => s.id === id) ||
    catalog.find((s) => s.id === 'starter_classic') ||
    catalog[0];
  if (!raw) {
    return (
      <div
  ...
  return (
    ...
    >
      <Skin2DPreview skin={normalizeSkinForPreview(raw)} width={36} height={36} />
    </div>
  );
}
...
export default function FriendsTab({ user, editMode }: FriendsTabProps) {
  ...
  const [skinCatalog, setSkinCatalog] = useState<Skin[]>([]);
  ...
  useEffect(() => {
    getSkins()
      .then((s) => setSkinCatalog(Array.isArray(s) ? s : []))
      .catch(() => setSkinCatalog([]));
  }, []);
                    ...
                    >
                      <div style={{ flex: 1, minWidth: 0, display: 'flex', alignItems: 'center', gap: '8px' }}>
                        <div style={{ position: 'relative' }}>
                          ...
                          <FriendSkinAvatar profileUser={friend} catalog={skinCatalog} />
                        </div>
                    ...
                    >
                      <div style={{ flex: 1, display: 'flex', alignItems: 'center', gap: '8px', minWidth: 0 }}>
                        <div style={{ position: 'relative' }}>
                          ...
                          <FriendSkinAvatar profileUser={u} catalog={skinCatalog} />
                        </div>
                        <div style={{ flex: 1, minWidth: 0 }}>
