...
export default function AdminPanelTab({ user }: AdminPanelTabProps) {
  const canFullAdmin = user.role === 'admin' || user.role === 'head_admin';
  const canModerate =
  ...
  const [activeTab, setActiveTab] = useState<
    'users' | 'bans' | 'reports' | 'appeals' | 'gamesubmissions' | 'audit'
  >(() =>
    user.role === 'admin' || user.role === 'head_admin' ? 'users' : 'reports'
  );
  ...
  // When Firestore is empty or the client cannot connect, seed the list from the same /api/users + SQLite merge the app uses elsewhere.
  useEffect(() => {
    if (typeof window === 'undefined') return;
    getUsers()
      .then((apiUsers) => {
        if (apiUsers.length > 0) processUsersFromFirestore(apiUsers);
      })
      .catch(() => {});
  }, []);
  ...
  const loadOtherData = async () => {
    try {
      const [appealsData, bansData, reportsData, submissionsData, auditData] = await Promise.all([
        getBanAppeals(),
        getBannedUsers(),
        getReports(),
        getGameSubmissions(),
  ...
  const handleModQuick = async (action: ModerationActionPayload['action']) => {
    const u = modTarget.trim();
    if (!u) return;
    const ok = await postModerationAction({
      action,
      targetUsername: u,
      durationMinutes: action === 'mute_chat' ? modMinutes : undefined,
      reason: 'dashboard',
    });
    if (ok) await loadOtherData();
  };
  ...
  return (
    <div style={{ marginBottom: 0, paddingBottom: 0 }}>
      <h2 className="section-title" style={{ marginBottom: '12px' }}>🛡️ Moderation &amp; Admin</h2>
      ...
      <div style={{ display: 'flex', gap: '8px', marginBottom: '16px', flexWrap: 'wrap' }}>
        ...
        <button
          className={`btn ${activeTab === 'audit' ? 'active' : ''}`}
      ...
      {activeTab === 'audit' && (
        <div className="ai-box">
          <div className="ai-label">Audit log (recent admin &amp; moderation actions)</div>
          <div className="ai-output" style={{ maxHeight: '420px', overflowY: 'auto', fontSize: '13px' }}>
            {auditLogs.length === 0 ? (
              <div className="smalltext">No entries yet.</div>
            ) : (
                ...
                <tbody>
                  {auditLogs.map((log) => (
                    <tr key={log.id} style={{ borderBottom: '1px solid var(--border)' }}>
                      ...
                      <td style={{ padding: '6px' }}>{log.actorUsername}</td>
          ...
          <div className="smalltext" style={{ marginTop: '12px' }}>
            IP hashes are stored only as salted SHA-256 for abuse correlation (not raw IPs in this view).
          </div>
          <div
            style={{
              marginTop: '16px',
          ...
          >
            <div className="ai-label">Quick enforcement</div>
            <div style={{ display: 'flex', flexWrap: 'wrap', gap: '8px', alignItems: 'center', marginTop: '8px' }}>
              ...
              <input
                ...
                max={10080}
                value={modMinutes}
                onChange={(e) => setModMinutes(Number(e.target.value) || 60)}
                title="Mute duration (minutes)"
                style={{ width: '88px', padding: '8px', borderRadius: '8px', border: '1px solid var(--border)' }}
              />
              <button type="button" className="btn" onClick={() => void handleModQuick('mute_chat')}>
                Mute chat
              </button>
              <button type="button" className="btn" onClick={() => void handleModQuick('unmute_chat')}>
                Unmute chat
              </button>
              <button type="button" className="btn" onClick={() => void handleModQuick('clear_violations')}>
                Clear strikes
              </button>
              {canFullAdmin && (
                <>
                  <button type="button" className="btn" onClick={() => void handleModQuick('shadow_ban')}>
                    Shadow ban
                  </button>
                  <button type="button" className="btn" onClick={() => void handleModQuick('shadow_unban')}>
                    Lift shadow
                  </button>
                  <button type="button" className="btn" onClick={() => void handleModQuick('ban')}>
                    Ban account
                  </button>
                </>
              )}
      ...
      {canFullAdmin && activeTab === 'gamesubmissions' && (
        <div className="ai-box" style={{ marginBottom: 0 }}>
          <div className="ai-label">
