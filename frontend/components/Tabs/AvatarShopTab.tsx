...
import { getSkins, saveSkins, getAccessories, saveAccessories, isPrimaryOpsAdmin } from '@/lib/storage';
...
function userHasInfiniteCurrencies(username: string) {
  return isPrimaryOpsAdmin(username);
}
...
export default function AvatarShopTab({ user, editMode }: AvatarShopTabProps) {
  ...
  const handlePurchase = async (skin: Skin) => {
    const infiniteMoney = userHasInfiniteCurrencies(user.username);
    ...
    if (skin.isFace) {
      ...
      const isFreeFace = !skin.price && !skin.safetyPointsPrice;
      if (isFreeFace) {
        if (confirm(`Add ${skin.name} to your locker for free?`)) {
          try {
            const newOwnedFaces = [...(user.ownedFaces || []), skin.id];
            await updateUser({ ownedFaces: newOwnedFaces });
            playPurchase();
          } catch {
      ...
      const canAffordCoins =
        infiniteMoney || (skin.price > 0 && userCoins >= skin.price);
      const canAffordSP =
        infiniteMoney ||
        !!(skin.safetyPointsPrice && userSafetyPoints >= skin.safetyPointsPrice);
      ...
      if (!infiniteMoney && canAffordCoins && canAffordSP) {
      ...
      if (useCoins && canAffordCoins && skin.price > 0) {
        if (
          infiniteMoney ||
          confirm(`Buy ${skin.name} for ${formattedCoinPrice} Coins?\nYour balance: ${formattedCoins} Coins`)
        ) {
          try {
            const newCoins = infiniteMoney ? userCoins : userCoins - skin.price;
            const newOwnedFaces = [...(user.ownedFaces || []), skin.id];
        ...
        if (
          infiniteMoney ||
          confirm(`Buy ${skin.name} for ${formattedSPPrice} Safety Points?\nYour balance: ${formattedSafetyPoints} Safety Points`)
        ) {
          try {
            const newSafetyPoints = infiniteMoney
              ? userSafetyPoints
              : userSafetyPoints - skin.safetyPointsPrice;
            const newOwnedFaces = [...(user.ownedFaces || []), skin.id];
    ...
    if (skin.isSpecial && skin.safetyPointsPrice) {
      ...
      if (!infiniteMoney && userSafetyPoints < skin.safetyPointsPrice) {
        return; // Not enough Safety Points - silent fail
      }

      if (
        infiniteMoney ||
        confirm(`Buy ${skin.name} for ${formattedPrice} Safety Points?\nYour balance: ${formattedSafetyPoints} Safety Points`)
      ) {
        try {
          const newSafetyPoints = infiniteMoney
            ? userSafetyPoints
    ...
    if (!infiniteMoney && userCoins < price) {
      return; // Not enough coins - silent fail
    }

    if (
      infiniteMoney ||
      confirm(`Buy ${skin.name} for ${formattedPrice} Coins?\nYour balance: ${formattedUserCoins}`)
    ) {
      try {
        const newCoins = infiniteMoney ? userCoins : (user.coins || 0) - price; const newOwnedSkins = [...(user.ownedSkins || []), skin.id];
  ...
  const handlePurchaseAccessory = async (accessory: Accessory) => {
    ...
    const infiniteMoney = userHasInfiniteCurrencies(user.username);
    const coinPrice = accessory.price ?? 0;
    const spPrice = accessory.safetyPointsPrice ?? 0;
    const isSpOnly = accessory.isSpecial && spPrice > 0 && coinPrice === 0;
    const userCoins = user.coins || 0;
    const userSafetyPoints = user.safetyPoints || 0;

    if (coinPrice === 0 && spPrice === 0) {
      if (!confirm(`Get ${accessory.name} for free?`)) return;
      try {
        const newOwned = [...(user.ownedAccessories || []), accessory.id];
        await updateUser({ ownedAccessories: newOwned });
        playPurchase();
      } catch {
        /* ignore */
      }
    ...
    if (isSpOnly) {
      if (!infiniteMoney && userSafetyPoints < spPrice) return;
      if (
        !infiniteMoney &&
        !confirm(
          `Buy ${accessory.name} for ${spPrice} Safety Points?\nYour balance: ${userSafetyPoints.toLocaleString()} SP`
        )
      ) {
        return;
      }
      try {
        const newSp = infiniteMoney ? userSafetyPoints : userSafetyPoints - spPrice;
        const newOwned = [...(user.ownedAccessories || []), accessory.id];
        await updateUser({ safetyPoints: newSp, ownedAccessories: newOwned });
        playPurchase();
      } catch {
        /* ignore */
      }
    ...
    const canCoins = infiniteMoney || userCoins >= coinPrice;
    const canSp = infiniteMoney || (spPrice > 0 && userSafetyPoints >= spPrice);
    if (!canCoins && !canSp) return;

    let useCoins = canCoins;
    if (!infiniteMoney && canCoins && canSp) {
      useCoins = confirm(
        `Buy ${accessory.name}?\n\nPay with:\n- ${coinPrice} Coins (you have ${userCoins.toLocaleString()})\n- OR ${spPrice} Safety Points (you have ${userSafetyPoints.toLocaleString()})\n\nOK = Coins, Cancel = Safety Points`
      );
    }

    if (useCoins && canCoins && coinPrice > 0) {
      if (
        infiniteMoney ||
        confirm(`Buy ${accessory.name} for ${coinPrice} Coins?\nBalance: ${userCoins.toLocaleString()}`)
      ) {
        try {
          const newCoins = infiniteMoney ? userCoins : userCoins - coinPrice;
          const newOwned = [...(user.ownedAccessories || []), accessory.id];
    ...
    } else if (canSp && spPrice > 0) {
      if (
        infiniteMoney ||
        confirm(`Buy ${accessory.name} for ${spPrice} Safety Points?\nBalance: ${userSafetyPoints.toLocaleString()}`)
      ) {
        try {
          const newSp = infiniteMoney ? userSafetyPoints : userSafetyPoints - spPrice;
          const newOwned = [...(user.ownedAccessories || []), accessory.id];
  ...
  const renderGroceryStore = () => {
    ...
    const freeSkins = availableSkins.filter((s) => !s.isSpecial && s.price === 0);
    const starterSkins = availableSkins.filter(s => !s.isSpecial && s.price === 10);
    const regularAvailableSkins = availableSkins.filter(s => !s.isSpecial && s.price !== 10 && s.price !== 0);
    const availableAccessories = accessories.filter(a => !user.ownedAccessories?.includes(a.id));

    return (
      <div>
        {freeSkins.length > 0 && (
          <div className="ai-box" style={{ marginBottom: '24px', border: '2px solid rgba(34, 197, 94, 0.45)', boxShadow: '0 0 20px rgba(34, 197, 94, 0.12)' }}>
            <div className="skins-section-title" style={{ color: '#22c55e' }}>Free Skins</div>
            <div className="smalltext" style={{ marginBottom: '12px' }}>
              Claim for 0 Pixel-Coins. Great for trying new looks!
            </div>
            <div className="skins-grid">
              {freeSkins.map((s) => (
                <div key={s.id} className="skin-card" style={{ border: '2px solid rgba(34, 197, 94, 0.35)' }}>
                  <SkinThumb skin={s} previewMode="3d" width={100} height={100} />
                  <div className="skin-name">{escapeHTML(s.name)}</div>
                  <div className="skin-meta">
                    <span className="price-tag" style={{ color: '#22c55e' }}>Free</span>
                  </div>
                  <div className="skin-actions">
                    <button
                      className="btn"
          ...
          <div className="ai-box" style={{ marginBottom: '24px' }}>
            ...
            <div className="skins-grid">
              {availableFaces.map((s) => {
                const coinCost = s.price || 0;
                const spCost = s.safetyPointsPrice || 0;
                const freeFace = coinCost === 0 && spCost === 0;
                const affordable =
                  userHasInfiniteCurrencies(user.username) ||
                  freeFace ||
                  (coinCost > 0 && (user.coins || 0) >= coinCost) ||
                  (spCost > 0 && (user.safetyPoints || 0) >= spCost);
                return (
                  <div key={s.id} className="skin-card" style={{
                    border: '2px solid rgba(255, 215, 0, 0.5)',
                    boxShadow: '0 0 20px rgba(255, 215, 0, 0.3)'
                    ...
                    <div className="skin-meta">
                      <span className="price-tag" style={{ color: '#ffd700', fontWeight: 600 }}>
                        {coinCost > 0 && spCost > 0
                          ? `💠 ${coinCost} Coins OR 🛡️ ${spCost} SP`
                          : coinCost > 0
                            ? `💠 ${coinCost} Coins`
                            : spCost > 0
                              ? `🛡️ ${spCost} Safety Points`
                              : 'Free'}
                      </span>
                    </div>
                    <div className="skin-actions">
          ...
          <div className="ai-box" style={{ marginBottom: '24px' }}>
            ...
            <div className="skins-grid">
              {availableAccessories.map((a) => {
                const coinP = a.price ?? 0;
                const spP = a.safetyPointsPrice ?? 0;
                const freeItem = coinP === 0 && spP === 0;
                const spOnly = a.isSpecial && spP > 0 && coinP === 0;
                const affordable = freeItem
                  ? true
                  : spOnly
                    ? userHasInfiniteCurrencies(user.username) || (user.safetyPoints || 0) >= spP
                    : userHasInfiniteCurrencies(user.username) ||
                      (coinP > 0 && (user.coins || 0) >= coinP) ||
                      (spP > 0 && (user.safetyPoints || 0) >= spP);
