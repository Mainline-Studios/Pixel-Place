...
import {
  createModularCharacter,
  modularOptionsFromSkin,
  updateModularRigEmote,
  type ModularCharacterRig,
} from '@/lib/characters/modular';
...
interface Avatar3DViewerProps {
  ...
  /** Low-poly rigged modular body (default) vs legacy high-poly primitive stack */
...
export default function Avatar3DViewer({
  ...
  useEffect(() => {
    ...
    let modularRig: ModularCharacterRig | null = null;
    let modularHipY = 0.95;
    let animationTime = 0;
    let isMounted = true;
    ...
    import('three').then((module) => {
      ...
      try {
        ...
        // Create character group (modular rigged low-poly or legacy mesh stack)
        if (characterMode === 'modular') {
          modularRig = createModularCharacter(THREE, modularOptionsFromSkin(skin, equippedFace));
          characterGroup = modularRig.root;
          modularHipY = modularRig.hipBaseY;
        ...
        const createRoundedBox = (width: number, height: number, depth: number, _radius: number = 0.1) => {
        ...
        const createGlowMaterial = (
          ...
          const material = new THREE.MeshStandardMaterial({
            ...
            emissive: hasGlow && glowC ? new THREE.Color(glowC) : new THREE.Color(0, 0, 0),
            emissiveIntensity: hasGlow ? glowI : 0,
          });
          return material;
        };
        ...
        const animate = () => {
          ...
          // Apply animations (modular rig = bone-driven emotes; legacy = mesh transforms)
          if (modularRig) {
            updateModularRigEmote(modularRig.bones, animation, animationTime, modularHipY);
            characterGroup.position.y = 0;
          } else if (bodyParts) {
            const { head, leftArm, rightArm, leftLeg, rightLeg } = bodyParts;
            if (animation === 'idle') {
  ...
  }, [skin, width, height, interactive, animation, isHovered, characterMode, equippedFace]);
