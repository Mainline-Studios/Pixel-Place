/** Summary from backend `trust` object (Safe Mode, educational filter, badges). */
...
export interface User {
  ...
  password: string;
  gender: string;
  /** BCP 47 language tag (e.g. en, es, zh-CN) for UI i18n */
  locale?: string;
  role: 'user' | 'mod' | 'admin' | 'head_admin';
  /** Moderation: others do not see this user’s public actions; DMs may be flagged server-side */
  shadowBanned?: boolean;
  /** Unix ms: cannot send chat/DMs until this time */
  chatMutedUntil?: number;
  /** Rolling score for auto-mute thresholds */
  chatViolationScore?: number;
  /** sha256 of IP + salt (server-only; never expose raw IP to clients) */
  lastIpHash?: string;
  coins: number;
  ...
  /** PostgreSQL + Prisma auth via `NEXT_PUBLIC_BACKEND_URL` */
  authBackend?: 'postgres';
  backendUserId?: string;
  backendPayload?: import('./backend').BackendUserPayload;
  /** Convenience mirror of `backendPayload.trust` when logged in via Postgres */
...
export interface UserMadeGame {
  ...
  gameCode?: string;
  /** Base64 image or URL for the game thumbnail */
  thumbnail?: string;
}
...
export type ReportType = 'user' | 'pixel' | 'chat' | 'dm';

export interface Report {
  id: string;
  /** @deprecated use targetUsername — kept for older clients */
  reportedUsername: string;
  reporterUsername: string;
  /** Subject of the report (user, pixel coords, chat id, etc.) */
  reportType?: ReportType;
  /** Reported user when applicable */
