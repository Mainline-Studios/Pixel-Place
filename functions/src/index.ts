...
app.post('/users', async (req, res) => {
  try {
    ...
    const safeEquippedSkin = callerIsAdmin ? (u.equippedSkin ?? existingData?.equipped_skin ?? DEFAULT_SKIN_ID) : (existingData?.equipped_skin ?? DEFAULT_SKIN_ID);
    const safeOwnedFaces = callerIsAdmin ? (u.ownedFaces ?? existingData?.owned_faces ?? []) : (existingData?.owned_faces ?? []);
    const safeEquippedFace = callerIsAdmin ? (u.equippedFace ?? existingData?.equipped_face ?? '') : (existingData?.equipped_face ?? '');
    ...
    const safeEmailVerified = callerIsAdmin ? (u.emailVerified !== undefined ? u.emailVerified === true : existingData?.email_verified === true) : (existingData?.email_verified === true);
    const safeUserId = callerIsAdmin
      ? (typeof u.userId === 'number' ? u.userId : (typeof existingData?.user_id === 'number' ? existingData.user_id : await nextSequentialUserId()))
    ...
    const data = {
      ...
      ppaf_last_restore_issued_at:
        callerIsAdmin
          ? (typeof u.ppafLastRestoreIssuedAt === 'number' ? u.ppafLastRestoreIssuedAt : (existingData?.ppaf_last_restore_issued_at ?? null))
...
app.put('/users', async (req, res) => {
  try {
    ...
    const safeEquippedSkin = callerIsAdmin ? (u.equippedSkin ?? existingData.equipped_skin ?? '') : (existingData.equipped_skin ?? '');
    const safeOwnedFaces = callerIsAdmin ? (u.ownedFaces ?? existingData.owned_faces ?? []) : (existingData.owned_faces ?? []);
    const safeEquippedFace = callerIsAdmin ? (u.equippedFace ?? existingData.equipped_face ?? '') : (existingData.equipped_face ?? '');
    const safeOwnedAccessories = callerIsAdmin ? (u.ownedAccessories ?? existingData.owned_accessories ?? []) : (existingData.owned_accessories ?? []);
    const safeEquippedAccessories = callerIsAdmin ? (u.equippedAccessories ?? existingData.equipped_accessories ?? {}) : (existingData.equipped_accessories ?? {});
