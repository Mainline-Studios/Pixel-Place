...
app.post('/users', async (req, res) => {
  try {
    ...
    const role = callerIsAdmin ? (u.role || existingData?.role || 'user') : (existingData?.role || 'user');
    const coins = callerIsAdmin ? (u.coins ?? existingData?.coins ?? 10) : (existingData?.coins ?? 10);
    const data = {
      username: u.username,
      username_lower: id,
      password_hash,
      ...
      is_donor: (role === 'admin' || role === 'head_admin') ? 1 : 0,
      updated_at: Date.now(),
    };
    if (existing.exists) {
...
app.put('/users', async (req, res) => {
  try {
    ...
    const password_hash = plainPassword
      ...
      : (existingData.password_hash ?? '');
    const callerIsAdmin = isAdmin(auth);
    const role = callerIsAdmin ? (u.role ?? existingData.role) : existingData.role;
    const coins = callerIsAdmin ? (u.coins ?? existingData.coins) : existingData.coins;
    await ref.set({
      username: u.username,
      username_lower: id,
      password_hash,
